...
import { FlaskConical, Globe, BookOpen, Github, Database, Loader2, FileText, Sparkles, AlertCircle, Dna, Layers, Settings2, Info } from 'lucide-react'
...
export default function ResearchLab() {
    ...
    const [gapAnalysis, setGapAnalysis] = useState<{ topic: string, score: number, issues: string[] }[]>([])
    ...
    const runResearch = async () => {
        ...
        try {
            ...
            if (json.success) {
                ...
                setGapAnalysis([
                    { topic: 'Architectural Depth', score: 85, issues: ['Missing detail on service-to-service auth'] },
                    { topic: 'Edge Case Coverage', score: 62, issues: ['Network partition handling not explained', 'Retry backoff logic missing'] },
    ...
    const getSnippets = (): ResearchSnippet[] => {
        ...
        if (content.snippets && Array.isArray(content.snippets)) {
            return content.snippets.map((s: any) => ({
                ...
                content: typeof s === 'string' ? s : s.content || s.text || JSON.stringify(s).substring(0, 200),
                url: s.url
            }))
        }
        // Fallback: show raw data
        return [{ title: 'Research Output', source: 'raw', content: JSON.stringify(content, null, 2).substring(0, 1200) }]
    }
            ...
            <div className="flex items-center justify-between shrink-0">
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground text-xs">Multi-agent cross-source intelligence suite</p>
                    </div>
                </div>
            </div>
                ...
                <div className="w-80 shrink-0 space-y-4 flex flex-col overflow-y-auto pr-1">
                    <div className="card p-5 bg-muted/20 border-white/5 space-y-6">
                        ...
                        <div className="space-y-3">
                            <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Sources</label>
                            <div className="space-y-2">
                                {sourceOptions.map(opt => (
                                    <button
                                        key={opt.id}
                                        onClick={() => toggleSource(opt.id)}
                                        className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg border transition-all text-xs ${sources.has(opt.id)
                                    ...
                                    >
                                        <div className="flex items-center space-x-2">
                                            <opt.icon className={`h-4 w-4 ${sources.has(opt.id) ? opt.color : 'text-muted-foreground/50'}`} />
                                            <span>{opt.label}</span>
                                        </div>
                                        {sources.has(opt.id) && <div className="w-1 h-1 rounded-full bg-purple-400 shadow-[0_0_8px_rgba(192,132,252,0.8)]" />}
                                    </button>
                                ))}
                            </div>
                        </div>
                        ...
                        >
                            ...
                            <span className="text-sm font-bold">{isResearching ? 'RESEARCHING...' : 'LAUNCH'}</span>
                        </button>
                    ...
                    <div className="flex-1 bg-muted/10 border border-white/5 rounded-2xl overflow-y-auto p-6 scrollbar-thin">
                        {!results && !isResearching && (
                            <div className="h-full flex flex-col items-center justify-center text-center opacity-50 grayscale">
                                ...
                                <p className="text-sm max-w-xs mt-2">Enter a topic and select sources to begin multi-agent intelligence gathering.</p>
                            </div>
                        )}
                        ...
                        {isResearching && (
                            <div className="h-full flex flex-col items-center justify-center space-y-6">
                                ...
                                <div className="text-center">
                                    <p className="text-sm font-bold tracking-widest uppercase animate-pulse text-purple-400">Synchronizing Nodes</p>
                                    <p className="text-[10px] text-muted-foreground mt-2 font-mono">Querying {Array.from(sources).join(', ')} Cluster</p>
                                </div>
                            </div>
                        )}
                        ...
                        {results && !isResearching && (
                            <div className="space-y-6 animate-in slide-in-from-bottom-2 duration-300">
                                {getSnippets().map((snippet, i) => (
                                    <div key={i} className="card p-5 hover:border-purple-500/20 transition-all bg-black/20 group">
                                        <div className="flex items-start justify-between mb-3">
                                            <div className="flex items-center space-x-2">
                                                <FileText className="h-4 w-4 text-purple-400" />
                                                <h3 className="text-sm font-semibold group-hover:text-purple-400 transition-colors">{snippet.title}</h3>
                                            </div>
                                            <span className="text-[9px] uppercase tracking-tighter text-muted-foreground bg-white/5 border border-white/5 px-2 py-0.5 rounded-full">{snippet.source}</span>
                                        </div>
                                        <p className="text-sm text-muted-foreground leading-relaxed whitespace-pre-wrap line-clamp-6">{snippet.content}</p>
                                        {snippet.url && (
                                            <div className="mt-4 pt-3 border-t border-white/5">
                                                <a href={snippet.url} target="_blank" rel="noopener noreferrer" className="flex items-center space-x-1.5 text-[10px] text-purple-400 hover:underline truncate">
                                ...
                                gapAnalysis.map((gap: any, i: number) => (
                                    <div key={i} className="space-y-3">
                                        <div className="flex items-center justify-between text-xs mb-1">
                                            ...
                                            <span className={`font-mono text-[10px] ${gap.score > 80 ? 'text-emerald-400' : gap.score > 60 ? 'text-yellow-400' : 'text-red-400'}`}>{gap.score}%</span>
                                        </div>
                                        ...
                                        {gap.issues.length > 0 && (
                                            <div className="space-y-1.5">
                                                {gap.issues.map((issue: string, j: number) => (
                                                    <div key={j} className="flex items-start space-x-2 p-2 bg-red-500/5 border border-red-500/10 rounded-md">
