    ...
    const handleGuidedGenerate = async () => {
        ...
        try {
            const response = await apiService.callMCPTool('adn_skills', {
                ...
                tags: targetTags.split(',').map(t => t.trim()).filter(Boolean),
    ...
    const handleAdvancedCreate = async () => {
        ...
        try {
            ...
            const response = await apiService.callMCPTool('adn_skills', {
                ...
                tags: advancedTags.split(',').map(t => t.trim()).filter(Boolean),
            ...
            if (response.success && response.data) {
                ...
                setGeneratedSkill({
                    ...
                    tags: data.tags || advancedTags.split(',').map(t => t.trim()).filter(Boolean),
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="flex space-x-1 bg-muted p-1 rounded-lg w-fit">
                <button
                    onClick={() => { setCreatorMode('guided'); setStatus('idle'); setGeneratedSkill(null) }}
                    className={`flex items-center px-4 py-2 text-sm font-medium rounded-md transition-colors ${creatorMode === 'guided'
                ...
                <button
                    onClick={() => { setCreatorMode('advanced'); setStatus('idle'); setGeneratedSkill(null) }}
                    className={`flex items-center px-4 py-2 text-sm font-medium rounded-md transition-colors ${creatorMode === 'advanced'
                        ? 'bg-background text-foreground shadow-sm'
                        : 'text-muted-foreground hover:text-foreground'
                        }`}
                >
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {skillTypes.map(st => (
                                        <button
                                            ...
                                            className={`p-2 rounded-md border text-left text-xs transition-all ${skillType === st.id
                ...
                <div className="space-y-5">
                    <div className="bg-card border border-border rounded-lg p-5 space-y-4">
                        ...
                        <div>
                            ...
                            >
                                {showAdvancedParams ? <ChevronUp className="h-4 w-4 mr-1" /> : <ChevronDown className="h-4 w-4 mr-1" />}
            ...
            {status !== 'idle' && !generatedSkill && (
                <div className={`flex items-center p-4 rounded-lg border ${status === 'generating' ? 'border-blue-500/30 bg-blue-500/5' :
                    status === 'error' ? 'border-red-500/30 bg-red-500/5' : ''
                    }`}>
                    {status === 'generating' && <Loader2 className="h-5 w-5 mr-3 text-blue-400 animate-spin" />}
