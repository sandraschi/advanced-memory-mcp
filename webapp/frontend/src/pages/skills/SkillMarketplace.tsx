...
export default function SkillMarketplace() {
  ...
  const filteredOpenClaw = openclawSkills.filter(
      ...
      <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-left-4 duration-500">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            <code className="text-[10px] bg-white/10 px-1.5 py-0.5 rounded-md font-mono">
              {selectedSkill.toLowerCase().replace(/\s+/g, "-")}
            </code>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="card p-8 bg-gradient-to-br from-muted/20 to-transparent">
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground mt-1">
                      OpenClaw Workspace Skill · Version 1.0.0
                    </p>
                  </div>
                </div>
                <button
                ...
                >
                  {isImporting === selectedSkill ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
                  ) : importedSkills.has(selectedSkill) ? (
                    <CheckCircle className="h-4 w-4" />
                  ) : (
                    <Download className="h-4 w-4" />
                  )}
                  <span>
                    {importedSkills.has(selectedSkill) ? "Imported to ADN" : "Import Skill"}
                  </span>
                </button>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {scrubbingLog.map((log) => (
                    <div
                      key={log.id}
                      className="flex items-start space-x-2 p-2 bg-black/20 rounded border border-white/5"
                    >
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Browse ClawHub, manage local skills, and stay current with OpenClaw news
            </p>
          </div>
        </div>
        <div className="flex items-center space-x-2">
          <span
            className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${openclawOnline ? "bg-emerald-500/10 text-emerald-400" : "bg-red-500/10 text-red-400"}`}
          >
            <span
                ...
                <div className="card p-4 border-yellow-500/20 bg-yellow-500/5 flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Ensure <code className="text-[11px]">openclaw-molt-mcp</code> webapp_api is
                      running on port 10765.
                    </p>
                  </div>
                </div>
              )}
          ...
          {activeTab === "local" && (
            <div className="space-y-4">
              {filteredLocal.length === 0 ? (
                <p className="text-center text-muted-foreground py-12">
                  No local skills found{searchQuery ? ` matching "${searchQuery}"` : ""}. Import
                  some from ClawHub!
                </p>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
