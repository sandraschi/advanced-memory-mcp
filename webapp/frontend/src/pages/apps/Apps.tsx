...
import { LayoutGrid, Film, BookOpen, HardDrive, Monitor, Smartphone, Shield, Bot, ExternalLink, Activity, Loader2 } from 'lucide-react'
    ...
    return (
        <div className="space-y-8">
            ...
            <p className="text-muted-foreground max-w-2xl">
                Centralized launchpad for the entire MCP ecosystem. Each specialized agent-server exposes its own web interface on distinct ports for parallel operation and distributed intelligence.
            </p>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {APPS.map((app) => {
                    ...
                    return (
                        <div key={app.id} className="card group hover:shadow-glow transition-all flex flex-col h-full border border-white/5 hover:border-accent/30 overflow-hidden">
                            <div className="p-6 flex-1 flex flex-col">
                                <div className="flex items-start justify-between mb-4">
                                    <div className={`p-3 rounded-xl ${status === 'online' ? 'bg-accent/10 text-accent' : 'bg-muted text-muted-foreground'
                                        }`}>
                                    ...
                                    <div className="flex items-center space-x-2 bg-black/20 px-3 py-1 rounded-full border border-white/5">
                                        <div className={`w-2 h-2 rounded-full ${status === 'online' ? 'bg-green-500 animate-pulse' :
                                                status === 'checking' ? 'bg-yellow-500 animate-bounce' : 'bg-red-500'
                                            }`} />
                            ...
                            <a
                                ...
                                className={`flex items-center justify-center space-x-2 py-4 px-6 font-bold tracking-tight transition-all ${status === 'online'
                                ...
                                onClick={(e) => { if (status !== 'online') e.preventDefault(); }}
                            >
                                <span>Launch App</span>
                                {status === 'checking' ? <Loader2 className="h-4 w-4 animate-spin" /> : <ExternalLink className="h-4 w-4" />}
                            </a>
                        </div>
                    )
                })}
                ...
                <div className="flex items-start space-x-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gold/80 leading-relaxed max-w-3xl">
                            These sibling webapps are orchestrated via a common FastMCP backbone. All servers share the same authorization scope but maintain isolated SQLite storage and markdown vaults for maximum reductionist modularity. Cross-app data sharing is facilitated via the `inter_server` MCP communication layer.
                        </p>
                    </div>
                </div>
            </div>
