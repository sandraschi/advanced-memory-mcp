          ...
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                Monitoring active MCP instances across the reserved port grid (10700–10800+).
              </p>
            </div>
            <div className="flex items-center space-x-4">
              ...
              >
                <RefreshCw
                  className={`h-5 w-5 ${isScanning ? "animate-spin text-amber-500" : ""}`}
                />
              </button>
            </div>
          </div>
          ...
          {isScanning && apps.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
              {filteredApps.map((app) => (
                <div
                  key={app.port}
                  className="group relative bg-white/2 border border-white/5 hover:border-white/10 p-8 rounded-3xl transition-all duration-300 hover:bg-white/[0.04] hover:-translate-y-1"
                >
                  <div className="flex items-start justify-between mb-6">
                    ...
                    <div className="flex flex-col items-end space-y-2">
                      <div
                        className={`px-2 py-0.5 rounded-full text-[8px] uppercase font-bold tracking-widest border ${
                          app.status === "online"
                  ...
                  <div className="pt-6 border-t border-white/5 flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground opacity-50">
                        Encrypted
                      </span>
                    </div>
