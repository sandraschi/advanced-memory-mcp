...
import { BookOpen, Plus, Tag, Link as LinkIcon, Save, Trash2, Search, ChevronRight, Hash, Layers } from 'lucide-react'
...
export default function ZettelMaster() {
    const [zettels, setZettels] = useState<Zettel[]>([
        { id: '1', title: 'The Materialist Approach to Memory', content: 'Memory is not a ghostly storage but a physical reconfiguration of neural substrates.', tags: ['philosophy', 'neuroscience'], links: ['2'], timestamp: '2026-02-17' },
        { id: '2', title: 'MCP Implementation in Rust', content: 'FastMCP provides the necessary abstractions for high-performance agentic tools.', tags: ['dev', 'mcp', 'rust'], links: ['1'], timestamp: '2026-02-16' }
    ])
    ...
    const handleSave = () => {
        ...
        setZettels(prev => prev.map(z => z.id === selectedId ? {
            ...z,
            title: editTitle,
            content: editContent,
    ...
    const filteredZettels = zettels.filter(z =>
            ...
            <div className="flex items-center justify-between shrink-0">
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground text-xs">Structured note-making with bidirectional semantic mapping</p>
                    </div>
                </div>
                <button onClick={handleNew} className="btn btn-primary flex items-center space-x-2 shadow-lg shadow-emerald-500/20">
                    ...
                    <div className="flex-1 overflow-y-auto space-y-2 pr-1 scrollbar-thin">
                        {filteredZettels.map(z => (
                            <button
                                ...
                                className={`w-full text-left p-4 rounded-xl border transition-all group ${selectedId === z.id
                                    ? 'bg-emerald-500/10 border-emerald-500/30'
                                    : 'bg-muted/10 border-white/5 hover:border-white/10 hover:bg-white/5'
                                    }`}
                            >
                                <div className="flex items-start justify-between mb-2">
                                    <h3 className={`font-semibold text-sm truncate ${selectedId === z.id ? 'text-emerald-400' : 'text-foreground'}`}>{z.title}</h3>
                                    <span className="text-[9px] text-muted-foreground font-mono">{z.timestamp}</span>
                                </div>
                                <p className="text-xs text-muted-foreground line-clamp-2 mb-3 leading-relaxed">{z.content}</p>
                                <div className="flex flex-wrap gap-1">
                                    {z.tags.slice(0, 3).map(tag => (
                                        <span key={tag} className="text-[9px] px-1.5 py-0.5 rounded-full bg-white/5 border border-white/5 text-muted-foreground">#{tag}</span>
                                    ))}
                                    {z.tags.length > 3 && <span className="text-[9px] text-muted-foreground/50">+{z.tags.length - 3}</span>}
                                </div>
                            </button>
                        ))}
                    </div>
                ...
                <div className="flex-1 flex flex-col min-w-0">
                    {selectedId ? (
                            ...
                            <div className="card flex-1 flex flex-col overflow-hidden p-0">
                                ...
                                <div className="p-6 border-b border-white/5 flex items-center justify-between shrink-0">
                                    ...
                                    <div className="flex items-center space-x-2 ml-4">
                                        <button onClick={handleSave} className="p-2 hover:bg-emerald-500/10 text-emerald-400 rounded-lg transition-colors" title="Save changes">
                                            <Save className="h-5 w-5" />
                                        </button>
                                        <button onClick={() => handleDelete(selectedId)} className="p-2 hover:bg-red-500/10 text-red-400 rounded-lg transition-colors" title="Delete zettel">
                        ...
                        <div className="h-full card flex flex-col items-center justify-center text-center opacity-50 grayscale select-none">
                            ...
                            <p className="text-sm max-w-sm mt-2">Select a zettel to edit or create a new one to begin your semantic journey.</p>
                        </div>
                    )}
                </div>
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="flex items-center justify-between mb-1">
                                                <span className="text-[10px] uppercase font-bold text-emerald-400">Linked Note</span>
