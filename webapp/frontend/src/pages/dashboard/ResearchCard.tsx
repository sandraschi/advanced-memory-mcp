  ...
  return (
    <div className="flex items-start space-x-4 p-4 bg-muted/30 rounded-md hover:bg-muted/50 transition-colors">
      <div className={`${config.bgColor} p-2 rounded-md`}>
        <StatusIcon
          className={`h-4 w-4 ${config.color} ${research.status === "in_progress" ? "animate-spin" : ""}`}
        />
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between">
          ...
          <span
            className={`text-xs px-2 py-1 rounded-full ${config.bgColor} ${config.color} ml-2 flex-shrink-0`}
          >
            {config.text}
          </span>
        </div>
