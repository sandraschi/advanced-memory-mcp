...
export default function ControlRoom() {
  ...
  const [terminalLines] = useState<string[]>([
    "[SYSTEM] Initializing observability hooks...",
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      const data: AgentSession[] = [
        {
          id: "1",
          agent: "Advanced Memory Agent",
          status: "active",
          startTime: "19:12:18",
          duration: "12m 45s",
          toolsCalled: 14,
          tokens: "14,287",
          summary: "Researching materialist epistemology and implementing Phase 4 components.",
        },
        {
          id: "2",
          agent: "Robotics Orchestrator",
          status: "completed",
          startTime: "18:05:22",
          duration: "45m 12s",
          toolsCalled: 32,
          tokens: "42,102",
          summary: "Optimizing inverse kinematics for Unitree G1 platform.",
        },
        {
          id: "3",
          agent: "ClawHub Auditor",
          status: "completed",
          startTime: "17:30:10",
          duration: "8m 15s",
          toolsCalled: 6,
          tokens: "3,450",
          summary: 'Scrubbing external skill: "quantum-physics-expert" for Thermodynamic villains.',
        },
      ];
      setSessions(data);
      if (data.length > 0) {
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center space-x-2">
                      <div
                        className={`w-2 h-2 rounded-full ${s.status === "active" ? "bg-green-500 animate-pulse" : "bg-white/20"}`}
                      />
                  ...
                  <h4 className="text-xs font-bold mb-2 group-hover:text-amber-500 transition-colors line-clamp-1">
                    {s.summary}
                  </h4>
              ...
              <div className="flex-1 p-8 flex flex-col overflow-hidden">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    ...
                    <h3 className="text-[10px] uppercase font-bold tracking-[0.2em]">
                      Material Execution Stream
                    </h3>
                  </div>
                  <div className="flex items-center space-x-2 bg-black/60 border border-white/10 rounded-lg p-1">
                    <button
                      ...
                      title="Rewind Step"
                    >
                      <Rewind className="h-3.3 w-3.5" />
                    </button>
                    <button
                      className="p-1.5 hover:bg-amber-500/20 text-amber-500 rounded transition-colors"
                      title="Pause Stream"
                    >
                      <Pause className="h-3.3 w-3.5" />
                    </button>
                    <button
                      className="p-1.5 hover:bg-white/5 rounded transition-colors"
                      title="Terminate Session"
                    >
                      <Square className="h-3.3 w-3.5" />
                    </button>
                  </div>
                </div>
                ...
                <div className="flex-1 bg-black/80 rounded-2xl border border-white/10 p-6 font-mono text-[11px] overflow-y-auto scrollbar-thin text-muted-foreground space-y-2 relative group uppercase tracking-tight">
                  ...
                  {terminalLines.map((line, i) => (
                    <div key={i} className="flex space-x-3">
                      ...
                      <span
                        className={`${line.includes("[TOOL]") ? "text-blue-500" : line.includes("[LLM]") ? "text-purple-500" : line.includes("[ERROR]") ? "text-red-500" : ""}`}
                      >
                        {line}
                      </span>
                    </div>
                  ...
                  <div className="flex space-x-3 animate-pulse">
                    <span className="opacity-20">
                      [{terminalLines.length.toString().padStart(3, "0")}]
                    </span>
