...
import { Activity, Search, Info, Maximize2, RefreshCw, Globe, Zap, Filter, Layers } from 'lucide-react'
        ...
        const fetchGraph = async () => {
            ...
            try {
                ...
                if (response.success && response.data) {
                    ...
                    setGraphData({
                        nodes: [
                            { id: '1', name: 'Artificial Intelligence', group: 'core', val: 10 },
                            { id: '2', name: 'Machine Learning', group: 'topic', val: 7 },
                            { id: '3', name: 'Neural Networks', group: 'topic', val: 7 },
                            { id: '4', name: 'Deep Learning', group: 'topic', val: 7 },
                            { id: '5', name: 'NLP', group: 'topic', val: 6 },
                            { id: '6', name: 'Computer Vision', group: 'topic', val: 6 },
                            { id: '7', name: 'Ethical AI', group: 'research', val: 5 },
                            { id: '8', name: 'RAG Systems', group: 'research', val: 5 },
                        ],
                        links: [
                            { source: '1', target: '2' },
                            { source: '1', target: '3' },
        ...
        <div className="flex flex-col h-full bg-background overflow-hidden relative">
            ...
            <div className="absolute top-6 left-6 z-10 space-y-4 pointer-events-none">
                <div className="bg-black/40 backdrop-blur-xl border border-white/10 p-5 rounded-2xl pointer-events-auto">
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-4">
                        <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                            <span className="text-[9px] text-muted-foreground uppercase font-bold block mb-1">Total Nodes</span>
                            <span className="text-xl font-mono font-bold text-amber-500">{graphData.nodes.length}</span>
                        </div>
                        <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                            <span className="text-[9px] text-muted-foreground uppercase font-bold block mb-1">Relations</span>
                            <span className="text-xl font-mono font-bold text-blue-500">{graphData.links.length}</span>
                        </div>
                    </div>
                ...
                <div className="bg-black/40 backdrop-blur-xl border border-white/10 p-2 rounded-xl flex items-center space-x-1 pointer-events-auto">
                    <button className="p-2 hover:bg-white/5 rounded-lg transition-colors text-muted-foreground hover:text-white" title="Reset View">
                        <RefreshCw className="h-4 w-4" />
                    </button>
                    <button className="p-2 hover:bg-white/5 rounded-lg transition-colors text-muted-foreground hover:text-white" title="Auto Layout">
                        <Activity className="h-4 w-4" />
                    </button>
                    <button className="p-2 hover:bg-white/5 rounded-lg transition-colors text-muted-foreground hover:text-white" title="Toggle 3D View">
                        <Maximize2 className="h-4 w-4" />
                    </button>
                    <div className="w-px h-4 bg-white/10 mx-1" />
                    <button className="p-2 hover:bg-white/5 rounded-lg transition-colors text-muted-foreground hover:text-white" title="Graph Filters">
                ...
                {loading ? (
                    <div className="flex items-center justify-center h-full">
                        <div className="flex flex-col items-center space-y-4 opacity-50">
                            ...
                            <p className="text-xs uppercase tracking-[0.3em] font-bold">Simulating Neural Topography...</p>
                        </div>
                    </div>
                ) : (
                    <ForceGraph2D
                        ...
                        nodeCanvasObject={(node: any, ctx: CanvasRenderingContext2D, globalScale: number) => {
                            ...
                            ctx.fillStyle = node.group === 'core' ? '#f59e0b' : node.group === 'topic' ? '#3b82f6' : '#a855f7';
                            ctx.fill();
            ...
            <div className="absolute top-6 right-6 z-10 w-64">
                <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-5 overflow-hidden">
                    ...
                    <div className="flex flex-col items-center justify-center py-12 text-center border-2 border-dashed border-white/5 rounded-xl bg-white/2">
                        ...
                        <p className="text-[10px] text-muted-foreground">Select a node to inspect<br />semantic relationships</p>
                    </div>
