...
import { ArrowLeftRight, Upload, Download, CheckCircle, AlertCircle, Loader2 } from 'lucide-react'
...
export default function ImportExport() {
    ...
    const handleImport = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setResult({
                ...
                message: response.success ? 'Import completed successfully' : (response.error || 'Import failed')
            })
        } catch (error) {
    ...
    const handleExport = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setResult({
                success: response.success,
                message: response.success ? 'Export completed successfully' : (response.error || 'Export failed')
            })
        } catch (error) {
        ...
        <div className="max-w-4xl mx-auto space-y-8">
            ...
            <div className="card">
                <div className="flex border-b border-white/10">
                    <button
                        onClick={() => { setActiveTab('import'); setResult(null); setFormat(''); setPath(''); }}
                        className={`px-6 py-4 font-medium transition-colors relative ${activeTab === 'import' ? 'text-accent' : 'text-muted-foreground hover:text-foreground'
                            }`}
                    >
                        Import Data
                        {activeTab === 'import' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-accent" />}
                    </button>
                    <button
                        onClick={() => { setActiveTab('export'); setResult(null); setFormat(''); setPath(''); }}
                        className={`px-6 py-4 font-medium transition-colors relative ${activeTab === 'export' ? 'text-accent' : 'text-muted-foreground hover:text-foreground'
                            }`}
                    >
                        Export Data
                        {activeTab === 'export' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-accent" />}
                    </button>
                </div>
                ...
                <div className="p-8">
                    ...
                    {result && (
                        <div className={`mt-8 p-4 rounded-md flex items-start space-x-3 ${result.success ? 'bg-green-500/10 border border-green-500/20 text-green-400' : 'bg-red-500/10 border border-red-500/20 text-red-400'
                            }`}>
            ...
            <div className="grid gap-6 md:grid-cols-2">
                <div className="card p-6 border-l-4 border-accent">
                    ...
                    <p className="text-sm text-muted-foreground">
                        Imports are processed atomically. Each note becomes an Entity, and YAML frontmatter is converted to Observations and Tags. Existing notes with identical permalinks will be matched and updated if necessary.
                    </p>
                </div>
                <div className="card p-6 border-l-4 border-accent">
                    <h3 className="font-bold mb-2 flex items-center">
                    ...
                    <p className="text-sm text-muted-foreground">
                        Exports resolve all `[[Relation]]` links and embed linked images. The HTML and Docsify formats include a searchable index based on the Graph content.
                    </p>
                </div>
            </div>
        </div>
