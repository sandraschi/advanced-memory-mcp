...
export default function BatchImport() {
  ...
  const handleImport = async () => {
    ...
    setIsImporting(true);
    try {
      const response = await apiService.runBatchImport(
      ...
      <div className="flex items-center space-x-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Scan and ingest Markdown knowledge from external repositories
          </p>
        </div>
      </div>
      ...
      {scannedFiles.length > 0 && !results && (
        <div className="space-y-4 animate-in zoom-in-95 duration-300">
          <div className="flex items-center justify-between px-2 text-sm text-muted-foreground">
            ...
            >
              {selectedPaths.size === scannedFiles.length ? (
                <CheckSquare className="h-4 w-4 text-accent" />
              ) : (
                <Square className="h-4 w-4" />
              )}
              <span>
                {selectedPaths.size} of {scannedFiles.length} files selected
              </span>
            </button>
          </div>
          ...
          <div className="max-h-64 overflow-y-auto text-left space-y-2 mb-8 bg-black/20 p-4 rounded-md border border-white/5">
            {results.map((res, i) => (
              <div key={i} className="flex items-center space-x-2 text-xs">
                ...
                <span className={res.success ? "text-muted-foreground" : "text-red-400"}>
                  {res.path}
                </span>
              </div>
            ))}
