...
async def test_sync_status_completed():
    ...
    mock_tracker.get_summary.return_value = "[OK] All projects synced successfully"
    ...
    with patch("advanced_memory.services.sync_status_service.sync_status_tracker", mock_tracker):
        result = await mcp_fn(sync_status)()
...
async def test_sync_status_in_progress():
    ...
    mock_tracker.get_summary.return_value = "[SYNCING] Syncing 2 projects (5/10 files, 50%)"
...
async def test_sync_status_failed():
    ...
    mock_tracker = MagicMock(spec=SyncStatusTracker)
    mock_tracker.is_ready = False
    mock_tracker.get_summary.return_value = "[ERROR] Sync failed for: project1"
...
async def test_sync_status_idle():
    ...
    mock_tracker = MagicMock(spec=SyncStatusTracker)
    mock_tracker.is_ready = True
    mock_tracker.get_summary.return_value = "[OK] System ready"
    mock_tracker.get_all_projects.return_value = {}
...
async def test_sync_status_with_project():
    ...
    with patch("advanced_memory.services.sync_status_service.sync_status_tracker", mock_tracker):
        result = await mcp_fn(sync_status)(project="test-project")
...
async def test_sync_status_error_handling():
    ...
    with patch("advanced_memory.services.sync_status_service.sync_status_tracker") as mock_tracker:
        mock_tracker.is_ready = True
