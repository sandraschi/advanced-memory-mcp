...
def scaffold_skill(
    ...
    *,
    ...
    (modules_dir / "core-guidance.md").write_text(
        render_core_guidance(), encoding="utf-8"
    )
    ...
    (skill_dir / "scripts" / "example.py").write_text(
        render_example_script(slug), encoding="utf-8"
    )
    (skill_dir / "references" / "example.md").write_text(
        render_example_reference(title), encoding="utf-8"
    )
    (skill_dir / "assets" / "example.txt").write_text(
        render_example_asset(), encoding="utf-8"
    )
