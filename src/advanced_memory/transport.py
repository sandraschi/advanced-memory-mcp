...
Environment Variables:
    ...
    MCP_PORT: Port for HTTP/SSE. Default: 10704 (fleet 10700+; set MCP_PORT to override)
...
ENV_PORT = "MCP_PORT"  # default: 10704
...
def get_transport_config() -> dict:
    ...
    return {
        ...
        "port": int(os.getenv(ENV_PORT, "10704")),
...
Environment Variables:
  ...
  {ENV_PORT}         Port number (default: 10704)
...
Examples:
  ...
  # HTTP mode (web apps)
  python -m {server_name.replace("-", "_")} --http --port 10704

  # Via environment
  MCP_TRANSPORT=http MCP_PORT=10704 python -m {server_name.replace("-", "_")}
""",
    )
    ...
    parser.add_argument(
        "--port", type=int, default=None, help=f"Port to listen on (default: ${ENV_PORT} or 10704)"
    )
    parser.add_argument(
