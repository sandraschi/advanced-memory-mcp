...
async def export_joplin_notes(
    ...
    try:
        ...
        # Process the export
        result = await _process_joplin_export(notes_data, export_path_obj, create_notebooks)
...
async def _process_joplin_export(notes_data: list[dict[str, Any]], export_path: Path, create_notebooks: bool) -> str:
    ...
    for note_info in notes_data:
        try:
            ...
            exported_files.append({"title": note_info["title"], "error": str(e)})
...
def _generate_export_report(stats: dict[str, Any], exported_files: list[dict[str, Any]], export_path: Path) -> str:
    """Generate a comprehensive export report."""
    lines = [
        "# Joplin Export Complete",
    ...
    if exported_files:
        ...
        for file_info in exported_files[:20]:  # Limit to first 20
            if "error" in file_info:
                lines.append(f"- [UNICODE] **{file_info['title']}** - Error: {file_info['error']}")
            else:
                lines.append(
                    f"- [UNICODE] **{file_info['title']}** [UNICODE] {file_info['export_path']} (ID: {file_info['id']})"
                )
    ...
    lines.extend(
        [
            ...
            "1. **Open Joplin** and go to File [UNICODE] Import",
