...
async def export_html_notes(
    ...
    try:
        ...
        if combine_into_one:
            result = await _export_combined_html(notes_data, export_path_obj, html_title or "Combined Notes", make_toc)
            if show_after_export:
                ...
                combined_file = (
                    export_path_obj if export_path_obj.suffix == ".html" else export_path_obj.with_suffix(".html")
                )
...
""")
        ...
        if make_toc and toc_items:
            ...
            for item in toc_items:
                ...
                html_parts.append(f'                    <a href="#{item["anchor"]}">{item["title"]}</a>')
        ...
        for section in note_sections:
            ...
            html_parts.append(f'            <section id="{section["anchor"]}" class="note-section">')
...
def _generate_export_report(stats: dict[str, Any], export_path: Path, include_index: bool) -> str:
    ...
    lines.extend(
        [
            ...
            "- **CSS file:** styles.css (shared styling)",
        ]
    )
