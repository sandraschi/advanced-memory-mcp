...
│              Storage Substrate                       │
│                                                      │
│  - memory.db: SQLite (Entities, Obs, Rel)            │
│  - vectors/: LanceDB (Semantic Embeddings)           │
│  - projects/: Markdown storage isolation              │
└─────────────────────────────────────────────────────┘
                     │
                     │ File I/O
                     │
...
### Search & RAG
 ...
 #### Whoosh / SQLite FTS5
 **What**: Keyword-based full-text search
 **Why**:
-- No external dependencies (vs Elasticsearch)
-- Fast full-text indexing
-- Supports complex queries
-- Integrates with SQLAlchemy
-  - Native SQLite FTS5 for local speed
-  - Whoosh for complex boolean queries (Legacy/Fallback)
- 
- **Homepage**: https://whoosh.readthedocs.io/
- **Our Implementation**: `src/advanced_memory/services/search_service.py`
- 
- #### LanceDB
- **What**: Serverless vector database for AI
- **Why**:
- - **Hybrid Search**: Native support for combining FTS and Vector search.
- - **Disk-based**: Scales to millions of vectors without heavy RAM usage.
- - **Lance Format**: Extremely fast random access and scans.
- 
- **Docs**: [LanceDB Documentation](https://lancedb.github.io/lancedb/)
- 
- #### FastEmbed
- **What**: Light, fast Python library for generating embeddings.
- **Why**:
- - **No Heavy Dependencies**: Doesn't require full PyTorch/TensorFlow for inference.
- - **Quantized Models**: Uses ONNX Runtime for optimized CPU/GPU execution.
- - **Default Model**: `BAAI/bge-small-en-v1.5` for high-quality semantic retrieval.
- 
- **Docs**: [FastEmbed GitHub](https://github.com/qdrant/fastembed)
...
|---|---|
| `app.state.get("sampling_context")` | FastMCP has no `.state` attribute |
| `mcp.ctx.sample()` | `mcp.ctx` does not exist on FastMCP instances |
| `tools=[{"name": "x", "function": lambda ...}]` | Dict format is pre-2.14.1; pass callables directly |
| `context: Context = None` parameter | FastMCP injects by type annotation; convention is `ctx` |
| Manual `sample_step()` loop | Replaced by `ctx.sample(tools=[...])` which loops internally |

### Meta-tools vs Leaf Tools

- **Leaf tools** (`search_knowledge_base`, `read_knowledge_note`, etc.): plain Python, do real work, passed AS tools to `ctx.sample()` — they never call the LLM themselves.
- **Meta-tools** (`agentic_content_workflow`, `intelligent_batch_processor`): call `ctx.sample()` to let the LLM decide which leaf tools to call and in what order.

### Available Agentic Meta-tools

| Tool | Purpose |
|---|---|
| `agentic_content_workflow` | Multi-step knowledge workflows (search → read → synthesise) |
| `intelligent_batch_processor` | Batch processing of knowledge items with LLM orchestration |
| `sampling_capabilities_status` | Reports SEP-1577 implementation status and session availability |

Tool group registry (passed as `available_tools`): `search`, `read`, `write`, `browse`, `full`.
...
- LanceDB handles millions of vectors with disk-based storage
- Hybrid search (LanceDB + FTS5) optimized for sub-100ms retrieval
- Memory: FastEmbed uses ONNX for memory-efficient inference
- For >100,000 notes, hybrid architecture maintains constant-time retrieval performance
