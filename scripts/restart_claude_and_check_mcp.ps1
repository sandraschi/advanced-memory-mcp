...
if (-not $NoRestart) {
    ...
    if (-not $ClaudePath -or -not (Test-Path $ClaudePath)) {
        ...
        Write-Host "  python scripts\pre_commit_check.py" -ForegroundColor Yellow
        exit 1
    }
...
for ($i = $LastStartupIdx; $i -lt $LogLines.Count; $i++) {
    ...
    try {
        ...
        # Message format: "Registered X tools" or similar
        # Logger should match "advanced_memory" or "mcp"
        if ($Message -match "registered" -and ($Message -match "tool" -or $Message -match "portmanteau")) {
            # Make sure it's from tools/module (logger is like "advanced_memory.mcp.tools")
            if ($Logger -match "advanced_memory" -or $Logger -match "mcp" -or $Logger -match "tool") {
                $FoundSuccess = $true
