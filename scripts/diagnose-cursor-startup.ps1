...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "  âœ" Advanced Memory installed: $amVersion" -ForegroundColor Green
    } else {
        Write-Host "  âœ- Advanced Memory not installed" -ForegroundColor Red
...
try {
    ...
    if ($LASTEXITCODE -eq 0 -and $importTest -eq "OK") {
        Write-Host "  âœ" Server module imports successfully" -ForegroundColor Green
    } else {
        Write-Host "  âœ- Server module import failed" -ForegroundColor Red
...
if (Test-Path $cursorConfigPath) {
    Write-Host "  âœ" Configuration file found: $cursorConfigPath" -ForegroundColor Green
    ...
    if ($configContent.mcpServers.'advanced-memory') {
        Write-Host "  âœ" Advanced Memory configured in mcp_config.json" -ForegroundColor Green
        ...
        Write-Host "  âœ- Advanced Memory not configured in mcp_config.json" -ForegroundColor Red
        Write-Host "    Add configuration (see below)" -ForegroundColor Yellow
    }
} else {
    Write-Host "  âœ- Configuration file not found: $cursorConfigPath" -ForegroundColor Red
...
if (-not $testProcess.HasExited) {
    Write-Host "  âœ" Server started successfully (process running)" -ForegroundColor Green
    ...
    Write-Host "  âœ- Server exited immediately (check errors below)" -ForegroundColor Red
...
if (Test-Path $cursorLogPath) {
    Write-Host "  âœ" Cursor logs directory found: $cursorLogPath" -ForegroundColor Green
    ...
    Write-Host "  âš  Logs directory not found (may not exist until Cursor runs)" -ForegroundColor Yellow
}
