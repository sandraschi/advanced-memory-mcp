...
class DatabaseConsolidator:
    ...
    def run_analysis(self, scan_paths: list[Path] | None = None) -> dict:
        ...
        if not per_project_dbs:
            ...
            logger.info(f"\nTo clean up, run:")
            logger.info(f"  python scripts/consolidate_databases.py --clean-empty")
        else:
            ...
            logger.info(f"Per-project databases are legacy/unused.")
            logger.info(f"\nTo remove them, run:")
            logger.info(f"  python scripts/consolidate_databases.py --clean-empty")
...
Examples:
  ...
  python scripts/consolidate_databases.py --clean-empty --no-dry-run
    ...
    if args.clean_empty:
        consolidator.clean_empty_databases(
