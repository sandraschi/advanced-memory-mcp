...
from advanced_memory.schemas.project_info import ProjectList, ProjectStatusResponse
...
def display_project_info(
    ...
    try:
        # Get project info
        info = asyncio.run((project_info.fn if hasattr(project_info, "fn") else project_info)())  # type: ignore  # pyright: ignore [reportAttributeAccessIssue]
        ...
        if json_output:
            ...
            if info.statistics.most_connected_entities:  # pragma: no cover
                ...
                for entity in info.statistics.most_connected_entities:
                    connected_table.add_row(entity["title"], entity["permalink"], str(entity["relation_count"]))
