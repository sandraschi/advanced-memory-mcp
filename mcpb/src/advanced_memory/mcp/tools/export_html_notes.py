...
async def _process_html_export(
    notes_data: list[dict[str, Any]], export_path: Path, include_index: bool
) -> str:
    ...
    for note_info in notes_data:
        try:
            ...
            # Track for index
            folder_key = (
                str(folder_path.relative_to(export_path)) if folder_path != export_path else ""
            )
...
def _create_html_content(note_info: dict[str, Any]) -> str:
    ...
    markdown_content = note_info.get(
