...
class BasicMemoryPlugin {
  constructor() {
    this.name = 'advanced-memory-integration';
    this.version = '1.0.0';
  }
...
/advanced-memory export-notes --project current --format md

# Sync current document to Basic Memory
/advanced-memory sync-file --file "$current_file"

/advanced-memory create-links --content "$document_content"
```
...
```json
{
  "json_rpc": {
    ...
    "port": 8888,
    "host": "localhost",
    "allowed_origins": ["advanced-memory-app"]
  }
}
```
...
```json
{
  "commander": {
    "enabled": true,
    "custom_commands": {
      "basic_memory_sync": "/path/to/advanced-memory-cli sync $current_file",
      "basic_memory_export": "/path/to/advanced-memory-cli export --project current"
    }
  }
}
```
