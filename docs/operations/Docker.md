...
1. **Use the official image directly:**
   ...
   docker run -d \
     ...
     ghcr.io/basicmachines-co/advanced-memory:latest
   ```
...
2. **Or use Docker Compose with the pre-built image:**
   ...
   services:
     advanced-memory:
       image: ghcr.io/basicmachines-co/advanced-memory:latest
...
1. **Clone the repository:**
   ```bash
   git clone https://github.com/basicmachines-co/advanced-memory.git
   cd advanced-memory
   ```
...
```bash
# Build the image
docker build -t advanced-memory .
...
You can edit the advanced-memory config.json file located in the /root/.advanced-memory/config.json after Basic Memory starts.
...
You can edit the advanced-memory config.json file located in the /root/.advanced-memory/config.json
...
```bash
# Check status
docker exec advanced-memory-server advanced-memory status

# Sync files
docker exec advanced-memory-server advanced-memory sync

# Show help
docker exec advanced-memory-server advanced-memory --help
```
...
1. **Check current configuration:**
   ```bash
   docker exec advanced-memory-server cat /root/.advanced-memory/config.json
   ```
...
docker exec advanced-memory-server advanced-memory project create obsidian /app/data

# Set as default
docker exec advanced-memory-server advanced-memory project set-default obsidian
...
2. **Configuration Not Persisting:**
    - Use named volumes for `/root/.advanced-memory`
...
2. Review container logs: `docker-compose logs advanced-memory`
3. Verify volume mounts: `docker inspect advanced-memory-server`
4. Test file permissions: `docker exec advanced-memory-server ls -la /root`
...
Pre-built Docker images are available on GitHub Container Registry at [`ghcr.io/basicmachines-co/advanced-memory`](https://github.com/basicmachines-co/advanced-memory/pkgs/container/advanced-memory).
